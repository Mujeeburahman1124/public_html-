...
if ($res->num_rows === 1) {
    $row = $res->fetch_assoc();
    $up = $conn->prepare("UPDATE super_admins SET username=?, email=?, password_hash=? WHERE id=?");
    $up->bind_param('sssi', $username, $email, $hash, $row['id']);
    $up->execute();
    echo "Updated existing admin.\n";
} else {
    $in = $conn->prepare("INSERT INTO super_admins (username, email, password_hash) VALUES (?,?,?)");
